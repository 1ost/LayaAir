...
const temporaryDirectory = await mkdtemp(join(tmpdir(), "layaair-flash-stage-tests-"));
const output = join(temporaryDirectory, "flash-stage.test.mjs");

try {
    await build({
        entryPoints: [join(root, "tests/flashStage/flash-stage.runner.ts")],
        outfile: output,
        bundle: true,
        platform: "node",
        format: "esm",
