...
const temporaryDirectory = await mkdtemp(join(tmpdir(), "layaair-flash-strict-xml-"));
const runnerOutput = join(temporaryDirectory, "flash-strict-xml.test.mjs");
const browserOutput = join(temporaryDirectory, "flash-strict-xml.browser.js");
const browserHtml = join(temporaryDirectory, "index.html");

try {
    await build({
        entryPoints: [join(root, "tests/flashStrictXml/flash-strict-xml.runner.ts")],
    ...
    await build({
        entryPoints: [join(root, "tests/flashStrictXml/flash-strict-xml.browser.ts")],
    ...
    assert.equal(node.status, 0, `Node strict XML tests exited ${node.status}`);
    ...
    await writeFile(browserHtml,
        "<!doctype html><meta charset=utf-8><title>Strict XML browser gate</title><body><script src=flash-strict-xml.browser.js></script></body>",
        "utf8");
    ...
    const match = browser.stdout.match(/<pre id="flash-strict-xml-browser-result">([^<]*)<\/pre>/);
    assert.ok(match, `Strict XML browser marker missing:\n${browser.stderr}\n${browser.stdout.slice(-4000)}`);
