...
const temporaryDirectory = await mkdtemp(join(tmpdir(), "layaair-flash-events-tests-"));
const output = join(temporaryDirectory, "flash-events.test.mjs");

try {
    await build({
        entryPoints: [join(root, "tests/flashEvents/flash-events.runner.ts")],
        outfile: output,
        bundle: true,
        platform: "node",
        format: "esm",
