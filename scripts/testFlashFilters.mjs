...
const temporaryDirectory = await mkdtemp(join(tmpdir(), "layaair-flash-filter-tests-"));
const output = join(temporaryDirectory, "flash-filter.test.mjs");
try {
    await build({
        entryPoints: [join(root, "tests/flashFilters/flash-filters.runner.ts")],
        outfile: output,
        bundle: true,
        platform: "node",
        format: "esm",
