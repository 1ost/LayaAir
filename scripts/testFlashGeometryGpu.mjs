...
const temporaryDirectory = await mkdtemp(join(tmpdir(), "layaair-flash-geometry-gpu-"));
try {
    const bundle = join(temporaryDirectory, "flash-geometry-gpu.js");
    const html = join(temporaryDirectory, "index.html");
    await build({
        entryPoints: [join(root, "tests/flashGeometry/flash-geometry.gpu.ts")],
        outfile: bundle,
        bundle: true,
        platform: "browser",
        format: "iife",
    ...
    await writeFile(html, "<!doctype html><meta charset=utf-8><title>Flash geometry GPU gate</title><body><script src=flash-geometry-gpu.js></script></body>", "utf8");
    ...
    const match = result.stdout.match(/<pre id="flash-geometry-gpu-result">([^<]*)<\/pre>/);
    ...
    console.log(`Flash geometry GPU pixel gate passed (${payload.result.renderer})`);
