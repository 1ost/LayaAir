...
const temporaryDirectory = await mkdtemp(join(tmpdir(), "layaair-flash-timer-tests-"));
const output = join(temporaryDirectory, "flash-timer.test.mjs");

try {
    await build({
        entryPoints: [join(root, "tests/flashTimer/flash-timer.runner.ts")],
        outfile: output,
        bundle: true,
        platform: "node",
        format: "esm",
