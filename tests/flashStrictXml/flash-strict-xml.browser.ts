...
try {
    ...
    const document = StrictXmlDocument.parse(
        "<?xml version='1.0' encoding='UTF-8'?><!--p--><root a='&amp;&#65;'>x<item/><![CDATA[<&>]]><!--c--><item>z</item></root><!--e-->",
    );
    ...
    requireValue(document.root.children("item").length === 2, "child traversal parity failed");
    requireValue(document.root.descendants("item")[0] === document.root.childNodes[1], "identity parity failed");
    ...
    publish({ ok: true, result: { kinds: document.root.childNodes.map(node => node.kind), dtdRejected, accessorReads } });
} catch (error) {
