...
const HELP = `Usage: laya-authored-content <check|convert|publish> [options]
...
export async function main(arguments_: readonly string[]): Promise<0 | 1 | 2> {
    try {
        ...
        for (const [name, value] of [["project", parsed.values.project], ["workspace-root", parsed.values["workspace-root"]], ["provider-root", parsed.values["provider-root"]]] as const) {
            if (!value) throw new AuthoredContentToolError("AUTHORED_CONTENT_CLI_OPTION", `--${name} is required.`, { exitCode: 2 });
        }
        ...
        const exitCode = known ? error.exitCode : usage ? 2 : 1;
        const output = {
