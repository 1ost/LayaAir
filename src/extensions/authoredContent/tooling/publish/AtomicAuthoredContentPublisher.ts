...
export async function rollbackAuthoredContentGeneration(destinationRootValue: string, generation: string): Promise<void> {
    ...
    try {
        ...
        const receipt = JSON.parse(receiptBytes.toString("utf8")) as AuthoredContentConversionReceipt;
...
async function verifyGeneration(root: string, receipt: AuthoredContentConversionReceipt, pointer?: CurrentPointer): Promise<void> {
    ...
    const receiptBytes = await readFile(path.join(root, RECEIPT_FILE));
    ...
    const actual = JSON.parse(receiptBytes.toString("utf8")) as AuthoredContentConversionReceipt;
...
async function readCurrent(file: string): Promise<CurrentPointer | undefined> {
    try {
        const value = JSON.parse((await readFile(file)).toString("utf8")) as CurrentPointer;
