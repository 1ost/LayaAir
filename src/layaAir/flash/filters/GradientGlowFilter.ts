...
import {
    flashAngle, flashBlurDimension, flashFilterAlpha, flashQuality, flashRgb, flashStrength,
} from "../../laya/display/effect2d/FlashFilterCoercion";
...
interface GradientGlowFilterState {
    distance: number;
    angle: number;
    colors: number[] | null;
    alphas: number[] | null;
...
const GRADIENT_GLOW_FILTER_VALUES = new WeakMap<object, GradientGlowFilterState>();

function state(value: GradientGlowFilter): GradientGlowFilterState {
    const result = GRADIENT_GLOW_FILTER_VALUES.get(value);
    if (!result) throw new TypeError("Invalid GradientGlowFilter receiver");
    return result;
}

function requireArray(value: readonly number[] | null, name: string): readonly number[] {
    if (!Array.isArray(value)) throw new TypeError(`GradientGlowFilter.${name} must be a non-null Array`);
    return value;
}
...
/** Sealed Flash GradientGlowFilter backed by LayaAir's native directional alpha-field effect. */
export class GradientGlowFilter extends BitmapFilter {
    declare private readonly __gradientGlowFilterBrand: void;
    ...
    constructor(
        ...
        super();
        if (new.target !== GradientGlowFilter) throw new TypeError("GradientGlowFilter is not extensible");
        GRADIENT_GLOW_FILTER_VALUES.set(this, {
    ...
    equals(other: BitmapFilter | null): boolean {
        return isGradientGlowFilter(other)
...
export function isGradientGlowFilter(value: unknown): value is GradientGlowFilter {
    return typeof value === "object" && value !== null && GRADIENT_GLOW_FILTER_VALUES.has(value);
}

Object.freeze(GradientGlowFilter.prototype);
